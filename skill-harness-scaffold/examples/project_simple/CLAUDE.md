...
- Se detecta una posible regresión o comportamiento inesperado.
...
Al iniciar una sesión en todo-cli, el leader debe leer en este orden:

1. `AGENTS.md` — roles, capacidades y restricciones de cada agente.
2. `CHECKPOINTS.md` — estado actual del proyecto y próximo checkpoint pendiente.
3. `feature_list.json` — lista de features y su estado (pendiente / en progreso / hecho).
4. `docs/best-practices.md` — buenas prácticas obligatorias a seguir en cada tarea.
...
El archivo `docs/best-practices.md` define las convenciones y patrones que el leader debe seguir al planificar cualquier tarea. Su lectura es obligatoria en el protocolo de arranque (ver sección correspondiente).
...
El protocolo de leader (subagentes, restricciones de escritura, protocolo de arranque) **no aplica** en los siguientes casos:
